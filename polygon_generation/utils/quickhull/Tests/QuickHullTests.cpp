...
namespace tests {
  ...
  FloatType rnd(FloatType from, FloatType to) { return from + (FloatType)dist(rng) * (to - from); };

  static void assertSameValue(FloatType a, FloatType b) { assert(std::abs(a - b) < 0.0001f); }
  ...
  static void sphereTest() {
    ...
    // Test worst case scenario: more and more points on the unit sphere. All points should be part
    // of the convex hull, as long as we can make epsilon smaller without running out of numerical
    // accuracy.
    size_t i = 1;
  ...
  int run() {
    ...
    // Test 1 : Put N points inside unit cube. Result mesh must have exactly 8 vertices because the
    // convex hull is the unit cube.
    pc.clear();
    for (int i = 0; i < 8; i++) {
    ...
    assert(hull.getIndexBuffer().size() ==
           3 * 2 * 6);  // 6 cube faces, 2 triangles per face, 3 indices per triangle
    ...
    // Test 2 : random N points from the boundary of unit sphere. Result mesh must have exactly N
    // points.
    ...
    // Test 2.1 : Multiply x components of the unit sphere vectors by a huge number => essentially
    // we get a line
    ...
    for (size_t i = 0; i < 100; i++) {
      auto newp = centerPoint + vec3(rnd(-0.000001f, 0.000001f), rnd(-0.000001f, 0.000001f),
